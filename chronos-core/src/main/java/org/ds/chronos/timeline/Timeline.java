 ...
 * <p>
 * Given a {@link Chronicle}, {@link TimelineEncoder}, and
 * {@link TimelineDecoder}; we can produce a data layer for persisting and
 * fetching time stamped business objects.
 * 
 * @author Dan Simpson
 * 
 * @param <T>
...
public class Timeline<T> {
   ...
   * @param encoder
   */
  public Timeline(Chronicle chronicle, TimelineDecoder<T> decoder,
      TimelineEncoder<T> encoder) {
  ...
  /**
   * Fetch a lazy data stream for the time range, of which has stages that
   * ultimately produce objects of the output class
   * 
   * @param t1
   *          from time
   * @param t2
   ...
   * @param d2
   * @return
   * @see org.ds.chronos.api.Chronicle#getNumEvents(java.util.Date,
   *      java.util.Date)
   */
   ...
   * @param t1
   * @param t2
   * @see org.ds.chronos.api.Chronicle#deleteRange(java.util.Date,
   *      java.util.Date)
   */
